...
# =======================
# MAIN EXECUTION WRAPPER (Per la GUI)
# =======================
def esegui_previsioni(custom_matchdays=None, custom_odds=None):
    global DEBUG_MATCHDAYS
    
    # Aggiorna le variabili globali con i dati della GUI
    if custom_matchdays:
        DEBUG_MATCHDAYS = custom_matchdays
    ...
    try:
        ...
        if model is not None and scaler is not None:
            ...
            if not df_next.empty:
                # SE LA GUI HA PASSATO DELLE QUOTE CUSTOM, USA QUELLE
                if custom_odds:
                    # Facciamo un override "al volo" della funzione get_odds_mapping 
                    # solo per questa esecuzione.
                    global get_odds_mapping
                    get_odds_mapping = lambda: custom_odds
...
# Questo fa in modo che se avvii script_old.py da solo funzioni come prima,
# ma se lo importi da gui.py, aspetta che la GUI prema il tasto.
